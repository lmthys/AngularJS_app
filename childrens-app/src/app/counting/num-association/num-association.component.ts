...
@Component({
  selector: 'app-num-association',
  templateUrl: './num-association.component.html',
  styleUrls: ['./num-association.component.css'],
  encapsulation: ViewEncapsulation.None
})
export class NumAssociationComponent implements OnInit {
  ...
  newBoard(){
    ...
    for(var i = 0; i < 8; i++){
      ...
      for(var j = 0; j < 11; j++){
        if(this.cards[i].id == this.cards2[j].id){
  ...
  isThereIsAMatch() {
    return this.memory_tile_ids[0].substring(0,1) == this.memory_tile_ids[1].substring(0,1);
  }
